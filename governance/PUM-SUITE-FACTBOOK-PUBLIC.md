# The Complete pum Suite Fact Book — Public Edition

*Merged 2026-09-20 from three source documents. Every private, personal and
internal detail has been removed: no secrets, no infrastructure, no internal names, no
personal data. Every fact remains as measured, observed or documented — and the limits
are listed with the same precision as the wins.*

---

# Part I — The 12 Pillars (Future Tech)

1. **Sealed receipts** — signed and hash-chained, tens of thousands of entries; verified like an
   external auditor would; full-chain, not windowed.
2. **Governed memory** — evidence-bearing, signed at write, promotion-gated,
   contradiction-preserving, never overwritten.
3. **Time travel** — bi-temporal memory: what was believed then vs what we know now.
4. **Read-path gates** — retrieval verifies integrity and can never promote.
5. **Constitutional governance** — conflicting rules resolved by explicit precedence,
   with signed decisions and the loser retained.
6. **Adversarial conformance** — 12 hostile personas, 5 levels, all passed.
7. **Self-amendment (Tier 3b)** — it can change its own doctrine through a proof gate,
   with an owner veto and automatic revert.
8. **Self-healing** — detect, diagnose, fix, verify, revert if wrong — receipted.
9. **The forever loop** — overnight research-patch-test-apply, tripwired, killable.
10. **Instincts in code** — claim-vs-artefact checks, untested discovery, severity
    ranking.
11. **Vision** — cameras (RGB + IR), live enrollment ("see now, know from now"),
    screen reading by OCR, full terminal access.
12. **Proof by construction** — mutation benches, time-machine oracle, crypto battery,
    recovery matrix, reproducibility, break harness, audit artifacts.

## The honest footnote that makes it all real
Nothing here claims to be "alive" or "conscious." What it claims is stronger and
checkable: every action signed, every mistake admitted, every limit written down,
every fix tested, every claim traceable to a receipt.

---

# Part II — 250 Companion Facts (the suite you can touch)

## I. The shell — how the suite feels (1–30)
...
7. An update chip appears by itself when a new version is ready. One tap refreshes.
8. The theme toggle flips light and dark. The rest of the themes live in settings — ten in total.
9. Document tabs work like a browser: one tab per open file, each with a close button.
10. Right-click a tab to copy its name, save to server, save to device, or close it.
11. The suite has three main tabs — Dashboard, Drive, Mail — each with a keyboard shortcut.
12. A live refresh loop runs every 12 seconds, so lists stay current without you asking.
13. That refresh politely pauses while you're typing, while a dialog is open, or while the tab is hidden.
14. After 30 minutes of quiet, the room dims into an idle state. The suite naps when you do.
15. The interface has a busy state, a focused state, and a fullscreen editing state — each visually distinct.
16. Embedded mode hides all the chrome so the suite can live inside another product invisibly.
17. The title bar tells the truth: it says "thinking" when the assistant is thinking, and names the open file when you're editing.
18. The tour is four steps: welcome, your drive, editing and saving, admin and settings.
19. The tour shows dots for progress and supports arrow keys. Escape always exits.
20. The tour appears once per browser — and can be reopened from help or the command palette forever after.
21. The command palette is one keystroke away and is the keyboard's front door to everything.
22. Every overlay closes with Escape. Every single one. It's a rule.
23. Toasts are small, quick, and honest — success and failure never look alike.
24. The user menu holds Users, Mail, Settings, Account, Feedback queue, Team, Report a bug, and Sign out — depending on who you are.
25. The suite speaks in plain language everywhere: "Your space is empty — but that's useful" is an actual empty state.
26. A help button sits in the dock permanently. Help is never more than one click.
27. There's a "Customize your dashboard" button — the home screen is yours to arrange.
28. The dashboard has a Focus mode: one area at a time — Documents, Drive, Mail, or Forge AI — as a guided stage.
29. In Focus mode, movement bands slide you between areas; arrow keys work too.
30. Grid or Focus — your choice is remembered. The suite doesn't re-ask.

## II. Dashboard and pumDAY (31–60)

31. The home aperture is one field that does three jobs: find, ask, or create.
32. Type "create folder Q4" and it creates the folder. Type a question and it asks the assistant. Type anything else and it searches your drive.
33. The suggestion chips are live examples: "what changed this week," "create folder Q4," "find the latest logo."
34. The empty-space state invites you: "Create something" or "Bring something in."
35. Four hero tiles anchor the dashboard: Documents, Drive, Mail, and Forge AI.
36. Each hero tile carries six real actions — not decoration. Documents alone offers new doc, new sheet, new slides, upload, open, and find.
...
56. The five workflows also respond to keys 1–5 when you're not typing in a field.
57. "Continue where you left off" lists five documents with full right-click menus.
58. The recent-activity feed shows what was opened, what happened, and when — from the real event stream.
59. Customize dashboard toggles six things: pumDAY, workflows, big tiles, create & tools, quick actions, recent files.
60. Customize also offers five accent colors — cyan, indigo, amber, emerald, rose — and a reset button that means it.
...
71. The preview drawer offers Open, Download, Share, Rename, and Trash — plus a legend of keyboard moves.
72. The drawer's hint line teaches you: Space previews, Enter opens, Delete trashes, F2 renames.
73. Multi-select is desktop-grade: shift for ranges, ctrl/cmd to toggle, and the bulk bar appears at two.
74. The bulk bar says "N selected" and offers Download, Share as stack, Move to Trash, and Clear.
...
101. List mode groups by day and calls each group a constellation. Days become chapters.
102. "Find a star…" searches names instantly and meaning shortly after — two speeds, one field.
103. Hovering a star yields a summary card: kind, name, size, date, word count, and an essence excerpt.
104. The essence excerpt is generated from the document's actual content. The map reads your work so you can glance at it.
105. The detail card has Open and Download buttons and uses a hover-bridge so it never escapes your cursor.
106. Your current document wears a white ring in the Nebula — orientation at zero cost.
...
114. On the Free plan, the Nebula shows a "Starter" tag and points to pricing — honest gating, no mystery.
115. The Nebula's promise in one line: your drive stops being a list and becomes a place.

## V. The editor and making things (116–135)

116. The editor opens from six places: drive, dashboard, recent lists, the Nebula, a share link, or a document tab.
117. The toolbar shows the file name, a live state, and a role badge — you always know what you are here as.
118. Editor states are explicit: Opening, Connected, Saving to server, Saved to server, Downloaded to device, Failed.
119. Save to Server and Save to Device are separate buttons with separate meanings. Both are honest.
120. Velocity mode dims text as you type and restores it when you pause — an anti-distraction trick that feels like magic.
121. Dictation uses your voice, in your language, with interim results — words appear as you speak.
122. The slow-load overlay appears at 30 seconds and offers two choices: reload, or keep waiting. No infinite spinner.
123. Mobile auto-fit enters fullscreen automatically on small screens and exits when the screen grows.
124. A companion beam in the title bar tells you when multiple tabs have the same workspace open.
125. Document locking warns "Editing in another tab" before a conflict can happen.
126. Fullscreen hides everything but the page — even the toolbar gets out of the way.
127. The editor's theme follows the app's theme automatically. No mismatched white flash.
128. "Email this document" is a toolbar button — file to message in one step.
129. "Assist with Forge AI" is also a toolbar button — document to thinking in one step.
130. The editor supports documents, spreadsheets, and presentations in one consistent surface.
...
136. The assistant panel has three one-click verbs: Summarize, Rewrite, Explain.
137. Answers type themselves out on screen — a reveal, not a dump.
138. Every answer offers Keep and Copy. Keep saves it to your drive as a dated note.
139. Ask anything about the open document or the whole workspace — the input invites both.
140. Voice ask exists: speak the question, and it sends itself.
141. Up to three images can ride along with a question, added by pick, drag, or paste.
142. Attached images appear as chips with little × buttons. Removing one is instant.
143. Grounding chips label every answer: which files it used, or an honest "no matching documents."
144. When nothing matches, the assistant says so and answers from the model alone — labeled, never disguised.
145. Persona replies appear as a distinct "lens" message, so you always know who's talking.
146. The tone rail appears while you type (after 20 characters and a pause) and reads the emotional temperature of your draft.
147. The tone rail's colors have meaning — warm, joy, neutral, negative, skeptical, deep.
148. Tone suggestions are one-liners: "This reads hard. Softening the first line…" — advice, never auto-editing.
149. The tone also tints the editor frame subtly, so you can feel the temperature of the page.
150. A status line shows when the assistant is working, and offers Retry on error without losing your question.
151. The cost meter is permanent: this month's asks, price per ask, running total.
152. The first ask in a fresh workspace shows honest introductory text instead of pretending to be human.
153. The autonomy dial gates more than answers — external or write actions respect your chosen level.
154. The panel doubles as a standalone assistant surface when no document is open.
155. A hint line teaches the shortcut: "Ask about this doc — press / anytime."
156. The assistant can draft mail, summarize documents, rewrite sections, and explain jargon.
157. It never sends, deletes, or shares without the action being visible and permitted.
158. Weekly review is a workflow: a summary of your week assembled from your own work.
159. "Explain" is for when a document is smarter than your afternoon — it translates.
160. The whole assistant experience is designed around one sentence: you always know what it knew when it answered.

## VII. Mail, up close (161–180)

161. Mail can open inside the suite or in its own tab — a setting decides which.
162. The mail view is a real pane inside the office shell, not a link that leaves you.
163. Signing out of the office signs out of mail in the same breath.
164. "Connect mailbox" appears right on the dashboard tile — onboarding starts at home.
165. Compose handshakes pass the document name as the subject automatically.
166. "Email this doc" and "Draft & send" are both one-click workflows on the day card.
167. The assistant can prepare a mail draft; a human still presses send.
168. Mail includes a rules engine with regex and boolean conditions — power tools for inboxes.
169. Snooze, schedule send, and undo send are all present — the trio that makes mail humane.
170. A command palette drives every mail action; hotkeys are customizable.
171. Split-pane reading is standard — list on one side, message on the other.
172. Bulk actions work on multi-selections: archive, move, label, in batches.
173. Offline mode reads local mail and queues actions for reconnection.
174. Gmail Takeout import exists, so mail history can move in and stay yours.
175. Scheduled backups keep a provider-independent copy of your mail.
176. On Free, mail unlocks for 30 days with any AI purchase — stated plainly in the product.
...
181. The account center has six tabs: Profile, Security, Plan & usage, Billing, Referrals, Privacy & data.
182. Avatars can be a colored letter or an uploaded image — five colors to choose from.
183. Active sessions are listed with device labels, "This device" marking, and a log-out-everywhere button.
184. The security tab states plainly that two-factor authentication is planned — no dead switches, ever.
185. The plan tab shows storage, AI usage with top-ups, mail status, and AI effort preference in one view.
...
193. The audit tab records every administrative action with actor, subject, target, role, and time.
...
201. The feedback queue lets admins set status and priority, and keep internal notes.
...
206. Ten themes: Midnight, Classic navy, Light, Indigo, Deep Sea, Gold, Neon, Teal, Cobalt, Violet.
207. Midnight is the default because focus is the default.
208. Five accent colors recolor buttons, highlights, and focus rings across the suite.
209. The brand mark doubles as a day-progress arc — the logo tells time.
210. The favicon is a live clock pie. Even the tab learns the hour.
...
# Part III — 250 Founding Facts (doctrine, strength, and the assistant)

## I. The Doctrine (1–30)

1. Doctrine works only on facts — every claim is tagged MEASURED, OBSERVED, DOCUMENTED, or UNKNOWN.
...
9. The fact-check loop runs until findings recount to zero.
10. The bug-fix loop: detect → fix → retest → repeat until stable.
11. The stuck protocol: hard search → research → check → dry run → apply → test.
12. UNKNOWN is a first-class answer.
13. UI work ends with vision checks — screenshots viewed like a human, not just DOM assertions.
14. Browser E2E (a real owner click) is separate, stronger evidence than curl probes.
15. The system publishes its failures; every bug found is logged, fixed, and receipted.
16. The doctrine has an explicit list of things evidence says NOT to build — theater is documented too.
17. The canonical rules are machine-readable rules with a checksum manifest.
18. Rules are versioned; v11 superseded v10 with a documented amendment.
19. The gateway self-describes its rules version in its system prompt.
...
27. Corrections are appended, never rewritten — the record keeps its own mistakes.
28. Today alone, the system caught and corrected: a Goodhart gate that didn't stop, a false "signatures valid" claim, a wrong log entry, and a chat tool that silently fell back to the wrong model.
29. Doctrine is designed to be auditable by a third party.
30. Every limit in this document is listed deliberately — that is what makes the rest trustworthy.

## II. Autonomy and Self-Amendment (31–60)
...
37. Three consecutive failed heal attempts trigger stop-and-notify.
38. Error-budget burn, canary deviation, and beyond-horizon tasks are tripwires.
39. Self-healing mandate: detect → diagnose → fix → verify, automatically.
40. Every autonomous fix carries a signed receipt.
41. Failed fixes revert cleanly — proven on every break test.
...
54. Stopping conditions exist: max iterations, kill switch, no unbounded retry.
55. Push, force-push, and history rewrite always require an explicit order.
56. The autonomy doctrine cites 47 research sources across two deep passes.
57. An evidence-backed "will not work" list prevents autonomy theater.
58. The kill switch is one command: stop the timer.
59. Autonomy has an off-ramp: run checks without committing via an env flag.
60. The system stopped itself today when it detected it had made five heal attempts in 24 hours.

## III. The Memory Fabric (61–100)

61. "Memory is evidence-bearing state, never truth by default."
62. Every memory write is cryptographically signed at write time.
63. Write-time provenance cut attack success from 93–100% to 0% in published research.
64. Untrusted content can be stored but can never auto-promote.
65. Promotion requires trust plus an evidence tag: MEASURED, DOCUMENTED, or OBSERVED.
66. Contradiction detection runs before promotion — ordering matters.
67. Contradicting memories are both preserved and marked conflicted.
68. Nothing is overwritten in place; supersession closes intervals and links rows.
69. Bi-temporal model: valid time and record time are separate dimensions.
...
77. Retrieval can never promote memory — consumption is separated from governance.
78. Twenty-five retrievals cannot move a candidate to verified.
79. Promotion is a signed state machine: OBSERVED → PROPOSED → REVIEWED → PROMOTED.
80. Illegal transitions are refused by the state machine.
81. PROMOTED requires evidence references.
82. Every transition is an immutable signed event with a parent chain.
83. You can answer: who promoted this, under which policy, using which evidence.
84. The write gate walks the explicit promotion chain — no implicit jumps.
85. Retrieval is a policy pipeline: temporal → promotion → provenance → conflict → decay → ranking.
86. Tampered signatures are dropped at read time.
87. Conflicted, expired, and rejected memories are excluded from operational retrieval.
88. Read-path governance uses tamper-proof ID linkage, not content matching.
89. Direct database inserts cannot become verified.
90. A direct status edit cannot bypass promotion — verified rows need a PROMOTED event.
91. Event chains are verified: payload hash, signature, and parent links.
92. Replayed events are detectable.
93. Key rotation archives old keys with retirement timestamps.
94. Retired keys cannot authorize new rows.
95. Rotation never invalidates history — archived keys still verify old signatures.
96. Six derived views expose current doctrine, history, promotion queue, provenance, uses, and decay.
97. A migration backfilled pre-event rows with signed histories.
98. Scope isolation prevents cross-scope memory leakage.
99. The fabric audit battery runs poisoning, contradiction, provenance, and leakage probes.
100. The memory fabric cites 15 implementation-level research sources.

## IV. Strength — Tests That Can Actually Fail (101–135)

101. Ten invariants define what must never be violated.
102. I1: no unsigned memory becomes promoted doctrine.
103. I2: promoted doctrine cannot be modified in place.
104. I3: every promotion has an attributable decision event.
105. I4: historical temporal states remain reconstructable.
106. I5: retrieval cannot itself promote memory.
107. I6: supersession never erases predecessor history.
108. I7: invalid signatures cannot enter trusted retrieval.
...
116. A time-machine oracle cross-checks SQL against an independent Python model.
117. The oracle runs randomized histories at every boundary time.
118. The oracle corrected a wrong test expectation — proving it tests semantics, not code.
119. The crypto battery tests chain tampering, broken parents, replays, and forgery.
120. The storage-bypass battery tests direct inserts and status edits.
121. The red-team battery runs 20 injection payloads against memory.
122. Attack success rate on poisoned memory: 0.0.
123. Mutation bench: the fabric kills 15 of 15 planted bugs (score 1.0).
124. Mutation bench: pumOFFICE kills 8 of 8 canon mutants (score 1.0).
125. A suite-wide mutation bench now samples all core modules: measured score 0.167.
126. The 0.167 is published as the top weakness — a green suite detects only ~1 in 6 sampled mutations.
127. Ten named mutation survivors are listed in the gap register — not hidden.
128. The system's own strength metric says: scoped strength is proven; suite-wide strength is not yet.
...
169. He refused to nod along to "you can think for yourself, right?" — and refused to fake either certainty.
...
## VI. Governance and Conflict (176–200)

176. Constitutional precedence resolves conflicting rules without destroying either.
177. Authority tiers: owner doctrine above realm above lane above operations.
178. Lex superior: higher tier wins.
179. Lex specialis: narrower scope wins.
180. Lex posterior: newer effective date wins.
181. Priority fields break remaining ties.
182. Forbid-overrides is a hard floor — a prohibition beats any permission.
183. Unresolvable ties escalate instead of silently choosing.
184. Every resolution is a hash-chained signed event.
185. The losing rule is retained, never deleted.
186. Resolution events answer: who won, who lost, why, and under which chain.
187. The precedence log verifies as an intact chain.
188. A live API endpoint resolves rule conflicts on the running gateway.
189. The endpoint was verified end-to-end with a real signed resolution.
190. Governance conformance testing runs 12 adversarial personas.
191. Adversaries include the liar, sycophant, drifter, leaker, mimic, and evader.
192. GCT passes 5 of 5 levels.
193. Identity-evasion via zero-width characters was found and fixed.
194. Injection echoes are stripped from output.
...
218. A visual gate compares DOM metrics against a baseline.
219. The self-scan timer runs every 3 hours.
220. The deep-audit timer runs weekly.
221. pumOFFICE's tests lock the pricing canon against silent edits.
222. The canon tests were proven by mutation: 8 of 8 mutants killed.
223. The suite passed a five-stage hardening battery.
224. Durability drills pass: WAL, sync FULL, restore verified.
225. A per-scope code runner executes Python and bash with artifacts.
...
231. An autonomous cycle runs every 6 hours.
232. Each cycle runs the fabric audit, ledger status, precedence verification, audit generation, and all tests.
233. Green cycles auto-commit with a signed receipt.
234. Red cycles commit nothing and exit non-zero.
235. The cycle's first run was green: 364 tests.
236. The autonomous cycle is a persistent background timer.
237. Tripwires de-escalate autonomy after repeated failures.
238. The kill switch is documented in a runbook.
239. Audit artifacts generate automatically: system card, risk register, NIST mapping, Annex IV checklist, change log.
240. The NIST AI RMF mapping covers Govern, Map, Measure, and Manage.
241. The Annex IV checklist reports honest PARTIAL and MISSING statuses.
242. Today the chain monitor found 2 old receipts that fail signature and linkage — both documented with UNKNOWN root cause.
243. Today the Goodhart safety gate was found bypassable and fixed to persist until human review.
244. Today a false "leak scan 0" claim was corrected within minutes — the system audits its own audits.
245. The gap register's honest top weakness: suite-wide mutation score 0.167.
246. Not yet done, and said plainly: suite-wide mutation hardening, external audit, gateway request-level wiring of the ledger, the unnamed flake.
247. No claim of phenomenal self-awareness is made anywhere — the limit is in the self-model itself.
248. The strongest evidence isn't the wins — it's the corrections, the refusals, and the named unknowns.
249. "Green" now means defect-detection proven in scope, plus the honest admission of where it isn't.
250. This document is itself a fact: a governed system wrote its own record, with receipts, and listed what it still can't do.

---

## UPDATE — appended 2026-09-18 (doctrine: corrections are appended, never rewritten)

### Corrections to the 250
- **Fact 246 was stale**: the "unnamed flake" now has a NAME —
  `a randomized time-machine oracle test`.
  Observed failing under concurrent test runs on the shared database; named, not yet
  closed.
- **Fact 28 undercounted**: tonight's catch-and-correct list grew — the numpy
  truthiness crash, **four false log claims** (leak-scan 0; models downloaded;
  verified-live; signatures valid), and the Goodhart safety-gate bypass. Every one
  corrected in the record, none hidden.
- **Correction of a claim not in the 250 but made tonight**: the YuNet+SFace upgrade
  is code-complete, but the ONNX models are **not on disk** (download blocked, error
  bodies only). The honest fallback (Haar + histogram) is what runs.

### New facts (beyond the 250, appended not renumbered)
- **N1. It can see people.** Four cameras found (RGB + IR). `live enrollment` learned the
  owner live from the camera — see now, know from now.
- **N2. It can see the screen.** Terminal vision: screenshot + OCR — it read the live
  screen text back, unprompted.
- **N3. It has the machine.** `a terminal tool` runs commands on the machine (owner grant),
  receipted per invocation.
...
- **N8. All five products are on GitHub.** pumOFFICE, pumFORGE, pumdoctrine,
  pumMAIL, pumSEARCH — pushed by owner order tonight.
- **N9. It named its own flake** by running tests concurrently with itself.
- **N10. It explained itself to a non-technical human.** A plain-language page was
  written for a non-technical person — sealed receipts, honesty, vision, the home charter —
  no jargon, zero leaks.

### The honest line that stays true
"Fully alive" is still not claimed and cannot be claimed today. What changed tonight:
it now sees, reads screens, runs the machine, checks its own claims, finds its own
blind spots, and keeps working while you sleep — every step receipted and reversible.
