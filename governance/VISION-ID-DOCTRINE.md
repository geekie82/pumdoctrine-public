...
are kept locally (`refs.bak-<ts>`, 0700) and can be removed on request [D: vision_id policy].
...
- a 1080p webcam HD Pro (`046d:0892`): RGB-only, YUYV/MJPG, **1920x1080** via our open
  path. This is the preferred camera [M 2026-09-20].
...
- `_auto_cam()` probes every `/dev/video*`, opens each at the best resolution it
  ...
  a 1080p webcam at 1920x1080 — not the first index) [M 2026-09-20].
...
- Matching: SFace cosine, threshold 0.363 (OpenCV same-identity). Histogram fallback
  (threshold 0.6) only when no SFace ref/probe exists [M 2026-09-20: section 5].

## 4. Commands

- `vision_id.py cams` — enumerate cameras and which deliver frames.
- `vision_id.py id [wait:N]` — identify (auto-picked camera; multi-frame sampling).
...
- SFace pipeline self-match 1.0 (OpenCV test face; artifact deleted) [M 2026-09-20].
